...
class Drive(Maneuver):
    ...
    def render(self, draw: DrawingTool):
        ...
        for i in range(200, int(ground_distance(self.car.pos, self.target_pos)), 200):
            target_dir = ground_direction(self.car.pos, self.target_pos)
            pos = draw_ground(self.car.pos) + target_dir * i
            draw.triangle(pos, target_dir)

        # speedometer
        # with the Psyonix API rendering occlusion, this is invisible :-(
        # car = self.car
        # width = 5
        # length = 40
        # forw_offset = 20
